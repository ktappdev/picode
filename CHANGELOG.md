...
- **`revive_closed_session` — bring a stopped worker back on duty.** Resumes a worker's own Pi session (`--session`) in a fresh or reclaimed Herdr pane with full tools, its journal, its mailbox, and any replies it still owed, then hands it the continuation in one call. Distinct from `picode_round_table` (read-only, one question, shuts down) and from the Restate `deliverDue` wake (which boots a _blank_ agent that only has the due envelope). Coordinator-only, one worker at a time, and requires a `task` — the point is a specific continuation, not an aimless resurrection. The result reports `context_age_minutes` and `head_moved_since_exit` so a stale revival is visible before it does damage. Pass `dry_run=true` to inspect a candidate without opening a pane.
- **`picode_finish` — worker handoff notes.** Workers close an assigned task with one call that both records a durable handoff (`outcome`, `changed`, `leftUnverified`) and sends the final report. The note is written before the send, so a failed report cannot lose the record, and it is authored by the worker rather than the coordinator — the coordinator knows what it asked for, the worker knows what happened. Stored at `.picode/picodes/<id>/handoff.json` (per-picode, so no shared-file write races and purge takes it away).
- **Recent workers ledger.** The coordinator's context now carries a bounded digest of the most recent workers: derived area (the directories each worker actually touched, read out of its session JSONL), liveness, context age, whether commits landed since it stopped, and what its handoff left unverified. This is the discovery layer that makes `revive_closed_session` reachable at all — without it the coordinator has no way to know which dead worker knows what. Deliberately derived, never authored: a worker whose pane was killed still appears. The `--picode-revived` flag carries the stop timestamp into a resumed session, which is told plainly that its context predates the gap and that it must re-read before editing.
...
- **`npm run test:unit` no longer hangs when the suite runs inside a Herdr pane.** The suite inherited `HERDR_ENV`/`HERDR_WORKSPACE_ID` from the developer's shell, so the two tests that flip `HERDR_ENV=1` to get past the coordinator's must-run-in-herdr guard also satisfied `startHerdrListener`'s guard and opened a real subscription to the live daemon — a socket nothing closed, which kept libuv alive so `node --test` never exited after reporting every test green. `test/unit.test.ts` now clears the Herdr env in `beforeEach` and restores it in `afterEach`, and `herdr/listener.ts` exports `getListenerHandle()` so a started listener can be released without a `session_shutdown` (the only path that stopped it before). Documented in AGENTS.md under "Test hermeticity".
...
- **Idle sit-reps are now bounded** — the periodic sit-rep wakes the coordinator with a full agent run over its whole context (measured at ~470k tokens of prefill per turn on a long-lived session), and it used to fire every 10 minutes for as long as any worker pane was still tracked — all night, on nothing. It now compares the picture it inspects (tracked panes, obligations, barriers, owed replies) against the last check: after 3 consecutive checks find no changes the timer stops itself, tells the operator once, and stays silent until an operator message or a delivered envelope re-arms it. Tune with `PICODE_SITREP_MAX_IDLE` (`0` = poll forever) alongside the existing `PICODE_SITREP_INTERVAL_MS`. A sit-rep run that changed nothing also skips its journal fork, so a no-op check costs one model call instead of two.
